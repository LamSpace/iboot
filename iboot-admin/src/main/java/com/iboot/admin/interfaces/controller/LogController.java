...
public class LogController {
    ...
    public Result<PageResult<LoginLogResponse>> loginLogList(
        ...
        if (username != null || ipAddress != null || status != null || beginTime != null || endTime != null) {
            logs = loginLogApplicationService.getLoginLogPageByCondition(
                    username, ipAddress, status, beginTime, endTime, pageNum, pageSize);
            total = loginLogApplicationService.countLoginLogsByCondition(
                    username, ipAddress, status, beginTime, endTime);
        } else {
    ...
    public void exportLoginLog(HttpServletResponse response,
        ...
        List<LoginLog> logs = loginLogApplicationService.getAllLoginLogsByCondition(
                username, ipAddress, status, beginTime, endTime);
    ...
    public Result<PageResult<OperateLogResponse>> operateLogList(
            @RequestParam(defaultValue = "1") Integer pageNum,
            @RequestParam(defaultValue = "10") Integer pageSize,
        ...
        if (title != null || operatorName != null || businessType != null || status != null || beginTime != null || endTime != null) {
            logs = operateLogApplicationService.getOperateLogPageByCondition(
                    title, operatorName, businessType, status, beginTime, endTime, pageNum, pageSize);
            total = operateLogApplicationService.countOperateLogsByCondition(
                    title, operatorName, businessType, status, beginTime, endTime);
        } else {
    ...
    public void exportOperateLog(HttpServletResponse response,
                                 ...
                                 @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime beginTime,
                                 @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime endTime) throws IOException {
        List<OperateLog> logs = operateLogApplicationService.getAllOperateLogsByCondition(
                title, operatorName, businessType, status, beginTime, endTime);
    ...
    public Result<PageResult<RunLogResponse>> runLogList(
            @RequestParam(defaultValue = "1") Integer pageNum,
        ...
        List<RunLogResponse> logs = runLogApplicationService.searchRunLogs(
                level, keyword, logger, thread, beginTime, endTime, pageNum, pageSize);
        long total = runLogApplicationService.countRunLogs(
                level, keyword, logger, thread, beginTime, endTime);
