...
public class LoginLogApplicationService {
     ...
     * 记录登录日志（异步执行）
     * <p>
     * 创建登录日志记录并保存到数据库，包括用户名、IP 地址、登录地点、
     * 浏览器信息、操作系统、登录状态等信息
     * </p>
     *
     * @param username 用户名
     ...
     */
    @Async
    public void recordLoginLog(String username, String ipAddress, String loginLocation,
                                String browser, String os, Integer status, String message) {
    ...
    public List<LoginLog> getLoginLogPageByCondition(String username, String ipAddress, Integer status,
        ...
        return loginLogRepository.findPageByCondition(username, ipAddress, status, startTime, endTime, pageNum, pageSize);
    }
     ...
     * @return 登录日志总数
     */
    public long countLoginLogsByCondition(String username, String ipAddress, Integer status,
                                           LocalDateTime startTime, LocalDateTime endTime) {
