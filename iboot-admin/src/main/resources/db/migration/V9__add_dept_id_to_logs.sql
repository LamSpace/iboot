...
ALTER TABLE sys_operate_log
    ADD COLUMN `dept_id` BIGINT NULL COMMENT '操作人员部门 ID' AFTER `dept_name`;
ALTER TABLE sys_operate_log
    ADD INDEX `idx_operate_dept_id` (`dept_id`);

-- 为登录日志表添加 dept_id 字段用于数据权限过滤
ALTER TABLE sys_login_log
    ADD COLUMN `dept_id` BIGINT NULL COMMENT '登录用户部门 ID' AFTER `username`;
ALTER TABLE sys_login_log
    ADD INDEX `idx_login_dept_id` (`dept_id`);
